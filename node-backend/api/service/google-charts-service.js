...
class SystemInformationDataAcess {
   ...
   */
  getMemoryDetails() {
    logger.info("SystemInformationDataAcess|getMemoryDetails ");
    try {
      return new Promise((resolve, reject) => {
        si.mem().then((result) => {
          ...
          logger.debug(
            "SystemInformationDataAcess|getMemoryDetails|graphData ",
            graphData
          );
          resolve(graphData);
        });
      ...
      console.log();
      logger.error(
        "Error Occured in SystemInformationDataAcess|getMemoryDetails ",
        err
      );
    }
  }
