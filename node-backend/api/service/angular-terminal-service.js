...
class WindowCommands {
  ...
  executeCommands(commands) {
    logger.info("WindowCommands|executeCommands|commands ", commands);
    try {
      return new Promise((resolve, reject) => {
        exec(commands, function (error, stdout, stderr) {
          if (error) {
            ...
            resolve({
              output: stdout,
              executedCommands: executeCommands,
              status: true,
            });
          }
        });
      ...
      logger.error("Error Occured in WindowCommands|executeCommands ", err);
    }
  }
  ...
  getExecutedCommands(commands) {
    logger.info("WindowCommands|getExecutedCommands ", commands);
    try {
      return new Promise((resolve, reject) => {
        resolve(executeCommands);
      });
    } catch (err) {
      logger.error("Error Occured in WindowCommands|getExecutedCommands ", err);
    }
  }
}
